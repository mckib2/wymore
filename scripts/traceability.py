...
def is_exercise_key(def_key):
    """Return True if the textbook JSON key refers to an exercise (exercise*.json)."""
    filename = def_key.split('/')[-1]
    return filename.startswith('exercise')

def write_traceability_table(f, def_key, def_data, traceability_map, counters):
    """Write one markdown table for a textbook JSON entry and update coverage counters."""
    def_filename = def_key.split('/')[-1]
    json_rel_path = f"textbook/{def_filename}.json"

    f.write(f"### `{def_key}` — {def_data.get('name', 'Unnamed Entry')}\n\n")
...
def generate_markdown_report(definitions, traceability_map):
    ...
    definition_keys = sorted([k for k in definitions.keys() if not is_exercise_key(k)])
    exercise_keys = sorted([k for k in definitions.keys() if is_exercise_key(k)])
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        def_counters = {'total_elements': 0, 'traced_elements': 0, 'faithful_elements': 0}
        for def_key in definition_keys:
            write_traceability_table(f, def_key, definitions[def_key], traceability_map, def_counters)

        f.write("## 2. Textbook Exercises\n\n")
        f.write("Curated exercise solutions live in [`Mbse/TextbookExercises/`](Mbse/TextbookExercises/); "
                "the registry in [`Registry.lean`](Mbse/TextbookExercises/Registry.lean) tracks scope.\n\n")

        exercise_counters = {'total_elements': 0, 'traced_elements': 0, 'faithful_elements': 0}
        witness_exercises = 0
        theorem_exercises = 0
        if exercise_keys:
            for def_key in exercise_keys:
                ex_kind = definitions[def_key].get('kind', 'witness')
                if ex_kind == 'theorem_proof':
                    theorem_exercises += 1
                else:
                    witness_exercises += 1
                write_traceability_table(f, def_key, definitions[def_key], traceability_map, exercise_counters)
            f.write(f"- Exercise kinds: **{witness_exercises}** witness, **{theorem_exercises}** theorem-proof\n\n")
        else:
            f.write("_No exercise JSON entries found in `textbook/`._\n\n")

        f.write("## 3. Completeness & Quality Summary\n\n")

        total_elements = def_counters['total_elements'] + exercise_counters['total_elements']
        traced_elements = def_counters['traced_elements'] + exercise_counters['traced_elements']
        faithful_elements = def_counters['faithful_elements'] + exercise_counters['faithful_elements']
        ...
        f.write(f"- **Total Structured JSON Entries**: {len(definitions)} "
                f"({len(definition_keys)} definitions/theorems, {len(exercise_keys)} exercises)\n")
        f.write(f"- **Definition/Theorem Elements**: {def_counters['total_elements']} "
                f"({def_counters['faithful_elements']} faithful)\n")
        f.write(f"- **Exercise Task Elements**: {exercise_counters['total_elements']} "
                f"({exercise_counters['faithful_elements']} faithful)\n")

...
\textit{Assertional:} $\Phi_{Z_{\mathrm{inc}}}$ is unchanged from the direct buildable; verifying the shift encoding means re-checking that same fragment (equivalently, exhibiting or verifying a homomorphism witness on finite abstractions, or checking the exhibited maps on $\mathbb{N}$).
